...
\textcolor{cyan}{first background on how extinction is used to infer the dust column density - nicest + other methods}.

% Some computer simulations show that structures appearing to be filamentary/cylindrical are sometimes more flattened, sheet-like structures viewed in projection \citep[e.g.,][]{Imara2021}. Recent observational work by \citet{Tritsis2022} and \citet{Rezaei2023} to infer the volume density structure of molecular clouds using dust maps supports this view. 


\textcolor{cyan}{surface density maps from observations of molecular clouds tend to reveal filamentary elongated structures with the highest-column density gas concentrated along a central ridge or spine. \citet{ImaraForbes_2023} model this global structure as a cylinder with a volume density distribution that goes as 
\begin{align}\label{eq:rho}
\textcolor{green}{\rho = \frac{\rho_0}{\left[1 + \left(\dfrac{r}{r_0}\right)^2\right]^{\alpha}}},
\end{align}
where $\rho_0$ is the peak density along the central spine of the cloud, $r_0$ is the ``turnover'' radius and $\alpha$ describes how quickly the density falls off beyond $r_0$}

\textcolor{cyan}{By fitting the surface density maps (using MCMC) \citet{ImaraForbes_2023} derive the spine parameters $\alpha$, $\rho_0$, and $r_0$ for the clouds in their sample. they find that clouds that have higher central densities tend to be compact (smaller turnover radii), but the fall-off index is insensitive to both r0 and rho0. \citet{ImaraForbes_2023} also found that (relationships with mass and tdep), implying that the  molecular clouds with () are further along in their evolution than }

\textcolor{cyan}{tension between volume density derived sfr/tdep vs from yso counts- this implies that (need to directly probe three--dimensional structure). overview of the different 3D maps available - leike, edenhofer, dharmwardena, wang.}
...
\caption{projected maps of a molecular cloud (perseus), with spines overplotted for 50,100, and 200 pc distance ranges}
\label{fig1:maps}
\end{figure*}
...
\caption{volume density profile along the spine (left) and as a function of perpendicular distance from the spine (right)}
\label{fig:profiles}
\end{figure}
...
\subsection{the relationships between mass and r, with fit parameters}
\textcolor{cyan}{we find that}
\textcolor{cyan}{\citet{ImaraForbes_2023} found that there is a }


\subsection{relationship of fit parameters (or sth derived from them) with tdep}
\textcolor{cyan}{we find that}
\textcolor{cyan}{\citet{ImaraForbes_2023} found that there is a }
...
truncation radii $R_T$. To delve into these potential relationships, we have found it helpful to plot the inferred density profiles as a function of radial distance
from the spine in Figure~\ref{fig:rho}.}

\textcolor{red}{While there is still a fair amount of variety between the clouds, some of which may come from the unknown inclination $i$, we see that, roughly speaking, the clouds'
